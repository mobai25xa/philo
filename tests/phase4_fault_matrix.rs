//! Deterministic Phase 4 fault-matrix contracts.

use bytes::Bytes;
use futures_util::StreamExt as _;
use http::{HeaderMap, HeaderValue, StatusCode, header};
use philo::provider::TestOnlyProfile;
use philo::transport::mock::{MockBodyItem, MockExchange, MockGate, MockResponse, MockTransport};
use philo::{GenerateRequest, LlmClient, LlmError, Message, ModelRef, RequestControl};

const ENDPOINT: &str = "http://127.0.0.1:41995/v1/chat/completions";
...
async fn deterministic_gate_proves_cancel_releases_a_blocked_body() {
    ...
    let runtime = TestOnlyProfile::localhost(ENDPOINT, "fault-fixture-key")
        .unwrap()
        .build()
        .unwrap();
