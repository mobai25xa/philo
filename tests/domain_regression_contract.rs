...
fn history_ids_are_unique_per_assistant_turn_and_normalization_is_idempotent() {
    let fixture = json_fixture("domain/history/planner-regression/repeated-id-across-turns.json");
...
fn missing_result_and_normalized_id_fail_or_succeed_at_the_history_boundary() {
    let missing = json_fixture("domain/history/planner-regression/missing-tool-result.json");
    ...
    let normalized = json_fixture("domain/history/planner-regression/normalized-tool-id.json");
...
fn schema_reference_cycles_and_depth_are_bounded() {
    for fixture in [
        "domain/schema/regression/self-ref.json",
        "domain/schema/regression/mutual-ref.json",
    ] {
    ...
    let error = ToolSchema::with_limits(
        json_fixture("domain/schema/regression/reference-depth-overflow.json"),
        limits,
    )
    .unwrap_err();
    assert_eq!(error.reason(), SchemaFailure::TooDeep);
...
fn schema_pointer_escaping_and_numeric_keywords_are_strict() {
    let schema = ToolSchema::new(json_fixture(
        "domain/schema/regression/escaped-pointer.json",
    ))
    .unwrap();
    schema
    ...
    for fixture in [
        "domain/schema/regression/fractional-max-items.json",
        "domain/schema/regression/negative-min-length.json",
    ] {
        let error = ToolSchema::new(json_fixture(fixture)).unwrap_err();
...
fn image_url_userinfo_is_rejected_without_retaining_the_url() {
    let fixture = image_fixture("domain/security/regression/image-userinfo.toml");
    assert_eq!(
    ...
    let redaction = image_fixture("domain/security/regression/redaction-canary.toml");
