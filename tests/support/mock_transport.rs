...
use philo::domain::LocalRequestId;
use philo::error::{ErrorStage, LlmError, TimeoutError};
use philo::provider::{RedirectPolicy, ResolvedEndpoint};
use philo::transport::{
...
impl Transport for MockTransport {
    fn execute(&self, request: HttpRequest) -> TransportFuture<'_> {
        Box::pin(async move {
            ...
            await_with_lifecycle(&lifecycle, sleep(exchange.start_delay)).await?;
            match exchange.outcome {
                MockOutcome::StartError(error) => Err(error),
                MockOutcome::Response(response) => {
                    let body = mock_body_stream(response.body, lifecycle, Arc::clone(&self.inner));
...
async fn await_with_lifecycle<F, T>(lifecycle: &RequestLifecycle, future: F) -> Result<T, LlmError>
where
