//! Endpoint, deployment, model mapping, and URL policy contracts.
...
use philo::provider::endpoint::{resolve_official, resolve_official_for};
...
const BASE: &str = "https://test.invalid/proxy?api-version=old";

fn source() -> CatalogSource {
    CatalogSource::new(
        CatalogSourceId::new("endpoint-mapping-contract-fixture").unwrap(),
        "2026-07-24",
        None::<String>,
    )
    .unwrap()
...
async fn compat_explicitly_controls_model_body_and_catalog_controls_endpoint() {
    ...
    let runtime = TestProvider::new("https://test.invalid/bootstrap", "test-key")
        .unwrap()
...
#[test]
fn official_endpoint_contract_is_unchanged() {
