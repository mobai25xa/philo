//! P2-014 fixture inventory, property, security, and integration contracts.
...
const API_KEY: &str = "philo-p2-fixture-key-canary";
...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/phase-2")
}
...
#[test]
fn phase2_fixture_tree_covers_required_directories() {
    ...
    for relative in [
        ...
        "streams/finish",
        "streams/malformed",
        "history/tool-pairing",
        "history/replay",
        "history/role-normalization",
        "history/multimodal",
        "schemas/valid",
        "schemas/invalid",
        "schemas/unsupported-keywords",
        "security/sensitive-arguments",
        "security/image-metadata",
        "security/opaque-thinking",
    ] {
        assert!(
...
#[test]
fn phase2_schema_fixture_matrix_covers_valid_invalid_and_remote_ref() {
    let valid_object = read_json("schemas/valid/object-required.json");
    let schema = ToolSchema::new(valid_object["schema"].clone()).unwrap();
    assert!(schema.is_strict_compatible());

    let nullable = read_json("schemas/valid/nullable-anyof.json");
    assert!(ToolSchema::new(nullable["schema"].clone()).is_ok());

    let array_enum = read_json("schemas/valid/array-enum.json");
    assert!(ToolSchema::new(array_enum["schema"].clone()).is_ok());

    let not_object = read_json("schemas/invalid/not-an-object.json");
    assert_eq!(
    ...
    let remote = read_json("schemas/unsupported-keywords/remote-ref.json");
    assert_eq!(
    ...
    let missing = read_json("schemas/invalid/missing-required-type.json");
...
#[test]
fn phase2_history_normalizer_is_idempotent_across_tool_scenarios() {
    let scenarios = [
        (
            "history/tool-pairing/success-call-result.json",
            vec![
        ...
        (
            "history/role-normalization/empty-assistant-removed.json",
            vec![
                Message::user("hi"),
    ...
    for (fixture, input) in scenarios {
        ...
        .unwrap();
        // P2-014 freezes message idempotence: re-normalizing output messages must not
    ...
    let empty_meta = read_json("history/role-normalization/empty-assistant-removed.json");
...
proptest! {
    ...
    #[test]
    fn phase2_history_normalizer_is_idempotent(
...
#[test]
fn phase2_history_fixture_errors_match_typed_failures() {
    let missing_meta = read_json("history/tool-pairing/missing-result.json");
    ...
    let duplicate_meta = read_json("history/tool-pairing/duplicate-result.json");
...
#[test]
fn phase2_thinking_replay_and_security_canaries_are_redacted() {
    let same_meta = read_json("history/replay/same-source-opaque.json");
    let cross_meta = read_json("history/replay/cross-model-opaque-drop.json");
    assert_eq!(same_meta["protocol"], "synthetic-p3-boundary");
    assert_eq!(cross_meta["protocol"], "synthetic-p3-boundary");
    ...
    let security = read_json("security/opaque-thinking/opaque-canary.json");
    assert_eq!(security["canary_opaque"], OPAQUE_CANARY);
}

#[test]
fn phase2_tool_arguments_are_not_logged() {
    let canary = read_json("security/sensitive-arguments/argument-canary.json");
...
#[test]
fn phase2_image_query_canary_is_redacted() {
    let canary = read_json("security/image-metadata/url-query-canary.json");
...
#[test]
fn phase2_tool_result_image_fixture_rejects_unsupported_content() {
    let meta = read_json("history/multimodal/tool-result-image-rejected.json");
...
async fn phase2_tool_roundtrip_keeps_execution_in_application() {
...
#[test]
fn phase2_structured_output_request_goldens_are_present() {
    for relative in [
        "requests/structured-output/json-object.json",
        "requests/structured-output/json-schema-strict.json",
    ] {
