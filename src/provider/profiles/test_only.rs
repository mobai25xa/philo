...
/// Crate-private localhost profile for unit and loopback transport tests.
#[derive(Clone, Debug)]
...
impl TestProvider {
    ...
    pub(crate) fn localhost(endpoint: &str, key: impl Into<String>) -> Result<Self, LlmError> {
    ...
    #[must_use]
    pub(crate) fn with_model_capabilities(mut self, profile: ModelCapabilityProfile) -> Self {
        self.profile
            .model_capabilities
    ...
    #[doc(hidden)]
    #[must_use]
    pub(crate) fn with_anthropic_messages(mut self) -> Self {
    ...
    #[must_use]
    pub(crate) fn with_auth_provider<A>(mut self, auth: A) -> Self
    where
    ...
    #[must_use]
    pub(crate) fn with_dynamic_header_policy(mut self, policy: DynamicHeaderPolicy) -> Self {
    ...
    #[doc(hidden)]
    #[must_use]
    pub(crate) fn with_endpoint_config(mut self, endpoint: EndpointConfig) -> Self {
    ...
    #[must_use]
    pub(crate) fn with_catalog(mut self, catalog: ModelCatalog) -> Self {
    ...
    #[must_use]
    pub(crate) fn with_compat(mut self, compat: CompatProfile) -> Self {
    ...
    #[must_use]
    pub(crate) fn with_model_compat(
        mut self,
    ...
    #[must_use]
    pub(crate) fn with_sse_config(mut self, config: SseConfig) -> Self {
    ...
    #[must_use]
    pub(crate) fn with_rate_limit_policy(mut self, policy: RateLimitPolicy) -> Self {
    ...
    #[must_use]
    pub(crate) fn with_idempotency_policy(mut self, policy: IdempotencyPolicy) -> Self {
        self.profile.idempotency = policy;
        self
    }

    /// Builds the test runtime.
    pub(crate) fn build(self) -> Result<ProviderRuntime, LlmError> {
