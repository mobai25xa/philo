...
impl CompatPatch {
    ...
    pub(crate) fn overlay(&mut self, later: &Self) {
        macro_rules! overlay {
    ...
    /// Applies this patch to a resolved profile.
    pub fn apply_to(&self, profile: &mut CompatProfile) {
        let (request, response, history, provenance) = profile.parts_mut();
        macro_rules! apply {
            ($slot:expr, $value:expr, $field:expr) => {
                if let Some(value) = $value {
                    $slot = value;
                    provenance.insert($field, self.source_for($field));
                }
            };
        }
        apply!(
