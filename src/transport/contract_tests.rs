...
use crate::test_http_server::{ResponsePlan, ScriptedServer as RunningServer};

use super::mock::{MockBodyItem, MockExchange, MockResponse, MockTransport};
...
fn request(
    ...
    let runtime = TestProvider::localhost(url, CANARY)
        .unwrap()
        .build()
        .unwrap();
    let headers = runtime
...
async fn mock_and_reqwest_share_the_http_byte_contract() {
    ...
    assert_eq!(inbound[0].header("content-type"), Some("application/json"));
    assert_eq!(
        inbound[0].header("authorization"),
...
async fn reqwest_drop_body_closes_the_incomplete_connection() {
    let server = RunningServer::spawn(vec![
        ResponsePlan::chunked(StatusCode::OK, [Bytes::from_static(b"first")]).incomplete(),
    ])
    .await;
...
async fn reqwest_concurrent_headers_do_not_cross_talk() {
    ...
    for request in &*inbound {
        ...
        assert_eq!(request.header("x-philo-request"), Some(marker.as_str()));
        assert_eq!(
...
async fn reqwest_redirect_policy_disables_cross_origin_and_limits_hops() {
    ...
    assert_eq!(
        same_origin_captured[1].header("authorization").unwrap(),
